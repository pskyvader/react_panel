...
var Utility = {
    ...
    toggle_leftbar: function() {
        ...
        if (menuCollapsed == "true") localStorage.setItem('collapsed_menu', "false");
        else if (menuCollapsed == "false") localStorage.setItem('collapsed_menu', "true");
    ...
    animateContent: function() {
        if ($.fn.velocity) {
            $('.animated-content .info-tile, .animated-content .alert, .animated-content .card, .animated-content .panel:not(.no-animate)').css('visibility', 'visible').velocity('transition.slideUpIn', {
                stagger: 50
            });
        }
    },
        ...
        plugin.init = function() {
            ...
            $('body').on('click', 'ul.acc-menu a', function() {
                ...
                if (!$('body').hasClass('sidebar-collapsed') || $(this).parents('ul.acc-menu').length > 1) {
                    if ($(this).siblings('ul.acc-menu:visible').length > 0) $(this).closest('li').removeClass('open');
                    else $(this).closest('li').addClass('open');
    ...
    enquire.register("screen and (max-width: 767px)", {
        ...
        unmatch: function() {
            ...
            if (!($('body').hasClass('sidebar-scroll'))) { //if not already added
                ...
                $('.static-sidebar.scroll-pane').nanoScroller({
                    stop: true
                });
            }
        }
