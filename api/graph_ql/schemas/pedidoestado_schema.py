...
class pedidoestado_schema(SQLAlchemyObjectType):
    class Meta:
        ...
        only_fields = (
            ["idpedidoestado"] + list(attribute.keys()) + list(read_only_attribute.keys())
        )


def resolve_pedidoestado(args, info, idpedidoestado, **kwargs):
    query = resolve(
        args, info, pedidoestado_schema, pedidoestado_model, idpedidoestado=idpedidoestado, **kwargs
    )
    return query.first()
...
all_pedidoestado = SQLAlchemyConnectionField(
    pedidoestado_schema, sort=graphene.String(), **attribute
)
...
class create_pedidoestado(graphene.Mutation):
    ...
    pedidoestado = graphene.Field(
        pedidoestado_schema, description="pedidoestado created by this mutation."
    )
    ...
    def mutate(self, info, input):
        pedidoestado = mutation_create(pedidoestado_model, input, "idpedidoestado",info)
...
class update_pedidoestado(graphene.Mutation):
    ...
    pedidoestado = graphene.Field(
        pedidoestado_schema, description="pedidoestado updated by this mutation."
    )
    ...
    def mutate(self, info, input):
        pedidoestado = mutation_update(pedidoestado_model, input, "idpedidoestado",info)
